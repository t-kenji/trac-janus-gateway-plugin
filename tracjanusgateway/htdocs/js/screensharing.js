						...
						janus.attach(
							{
								...
								success: function(pluginHandle) {
									...
									$('#screenmenu').removeClass('hidden').show();
									$('#createnow').removeClass('hidden').show();
	...
	janus.attach(
		{
			...
			onremotestream: function(stream) {
				if($('#screenvideo').length === 0) {
					...
					$('#screencapture').append('<video class="rounded centered hidden" id="screenvideo" width="100%" height="100%" autoplay/>');
				}
				// Show the video, hide the spinner and show the resolution when we get a playing event
				$("#screenvideo").bind("playing", function () {
					$('#waitingvideo').remove();
					$('#screenvideo').removeClass('hidden');
